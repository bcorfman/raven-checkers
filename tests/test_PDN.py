...
def test_parse_PDN_string_success():
    ...
    assert reader.games[0].event == "Casual Game"
    assert reader.games[0].date == "2019.11.26"
    assert reader.games[0].white_player == "Jubai"
    assert reader.games[0].black_player == "windycity"
    assert reader.games[0].result == "1-0"
    assert reader.games[0].moves == [[[10, 15], [23, 18]], [[7, 10], [22, 17]],
