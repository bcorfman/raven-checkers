...
class SetupBoard(Dialog):
    ...
    def body(self, master):
        ...
        self._npButton2 = Radiobutton(
            self._npLFrame, text="One", value=1, variable=self._num_players, command=self._enable_player_color
        )
        self._npButton2.pack(side=LEFT, pady=5, expand=1)
        self._npButton3 = Radiobutton(
            self._npLFrame, text="Two", value=2, variable=self._num_players, command=self._disable_player_color
        )
        ...
        self._rbColor1 = Radiobutton(self._playerFrame, text="Black", value=BLACK, variable=self._player_color)
        self._rbColor1.pack(side=LEFT, padx=0, pady=5, expand=1)
        self._rbColor2 = Radiobutton(self._playerFrame, text="White", value=WHITE, variable=self._player_color)
        ...
        self._rbTurn1 = Radiobutton(self._rbFrame, text="Black", value=BLACK, variable=self._player_turn)
        self._rbTurn1.pack(side=LEFT, padx=0, pady=5, expand=1)
        self._rbTurn2 = Radiobutton(self._rbFrame, text="White", value=WHITE, variable=self._player_turn)
        ...
        self._wmEntry = Entry(self._wmFrame, width=40, textvariable=self._white_men)
        ...
        self._wkEntry = Entry(self._wkFrame, width=40, textvariable=self._white_kings)
        ...
        self._bmEntry = Entry(self._bmFrame, width=40, textvariable=self._black_men)
        ...
        self._bkEntry = Entry(self._bkFrame, width=40, textvariable=self._black_kings)
        ...
        self._bsCheck = Checkbutton(
            self._bsFrame, variable=self._bsState, text="Start new game with the current setup?"
        )
        self._bsCheck.pack()
    ...
    def validate(self):
        ...
        if self.wm_list is None or self.wk_list is None or self.bm_list is None or self.bk_list is None:
            return 0  # Error occurred during parsing
        if not self._all_unique(self.wm_list, self.wk_list, self.bm_list, self.bk_list):
    ...
    def apply(self):
        ...
        if (
            sorted(self.wm_list) != sorted(self._orig_white_men)
