...
class BoardView(Observer):
    def __init__(self, root, **props):
        ...
        self.canvas = Canvas(root, width=self._board_side, height=self._board_side, borderwidth=0, highlightthickness=0)
        right_panel = Frame(root, borderwidth=1, relief="sunken")

        self.toolbar = Frame(root)
        font, size = get_preferences_from_file()
        self.scrollbar = AutoScrollbar(root, container=right_panel, row=1, column=1, sticky="ns")
        self.txt = Text(
            root, width=40, height=1, borderwidth=0, font=(font, size), wrap="word", yscrollcommand=self.scrollbar.set
        )
        ...
        self.bold = Button(name="bold", image=self.bold_image, borderwidth=1, command=self._on_bold)
        self.bold.grid(in_=self.toolbar, row=0, column=0, sticky=W)
        self.italic = Button(name="italic", image=self.italic_image, borderwidth=1, command=self._on_italic)
        self.italic.grid(in_=self.toolbar, row=0, column=1, sticky=W)
        self.bullets = Button(name="bullets", image=self.bullets_image, borderwidth=1, command=self._on_bullets)
        self.bullets.grid(in_=self.toolbar, row=0, column=2, sticky=W)
        self.numbers = Button(name="numbers", image=self.numbers_image, borderwidth=1, command=self._on_numbers)
        self.numbers.grid(in_=self.toolbar, row=0, column=3, sticky=W)
        self.addLink = Button(name="addlink", image=self.add_link_image, borderwidth=1, command=self._on_add_link)
        self.addLink.grid(in_=self.toolbar, row=0, column=4, sticky=W)
        self.remLink = Button(name="remlink", image=self.remove_link_image, borderwidth=1, command=self._on_remove_link)
        ...
        self.undo_all = Button(name="undo_all", image=self.undo_all_image, borderwidth=1, command=self._on_undo_all)
        self.undo_all.grid(in_=self.toolbar, row=0, column=7, sticky=W)
        self.undo = Button(name="undo", image=self.undo_image, borderwidth=1, command=self._on_undo)
        self.undo.grid(in_=self.toolbar, row=0, column=8, sticky=W)
        self.redo = Button(name="redo", image=self.redo_image, borderwidth=1, command=self._on_redo)
        self.redo.grid(in_=self.toolbar, row=0, column=9, sticky=W)
        self.redo_all = Button(name="redo_all", image=self.redo_all_image, borderwidth=1, command=self._on_redo_all)
        ...
        starting_squares = [
            i for i in self._model.curr_state.valid_squares if self._model.curr_state.squares[i] & (BLACK | WHITE)
        ]
    ...
    def _on_bullets(self):
        self._process_button_click(
            "bullet", self.bullets_tooltip, self._add_bullets_if_needed, self._remove_bullets_if_needed
        )

    def _on_numbers(self):
        self._process_button_click(
            "number", self.numbers_tooltip, self._add_numbers_if_needed, self._remove_numbers_if_needed
        )
    ...
    def init_tags(self):
        ...
        self.txt.tag_config("number", tabs=".5c center 1c left", lmargin1="0", lmargin2="1c")
        self.txt.tag_config("bullet", tabs=".5c center 1c left", lmargin1="0", lmargin2="1c")
    ...
    def reset_view(self, model):
        ...
        starting_squares = [i for i in sq if (self._model.curr_state.squares[i] & (BLACK | WHITE))]
        self._draw_checkers(Command(add=starting_squares))
        for i in sq:
    ...
    def calc_valid_xy(self, x, y):
        return (
            min(max(0, self.canvas.canvasx(x)), self._board_side - 1),
    ...
    def _draw_checkers(self, change):
        ...
        for i in change.add:
            ...
            self.canvas.create_oval(
                x, y, x + CHECKER_SIZE, y + CHECKER_SIZE, outline="black", fill=color, tags=(color, tag)
            )
            if checker & KING:
                self.canvas.create_image(x + 15, y + 15, image=self._crown_pic, anchor=CENTER, tags=(color, tag))
