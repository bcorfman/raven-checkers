...
class MainFrame(CenteredWindow):
    ...
    def undo_all_moves(self, *args):
        self.stop_processes()
        self.manager.model.undo_all_moves(None, self.manager.view.get_annotation())
    ...
    def create_game_menu(self):
        ...
        game.add_command(label="New game", underline=0, command=self.manager.new_game)
        game.add_command(label="Open game ...", underline=0, command=self.manager.open_game)
        game.add_separator()
        game.add_command(label="Save game", underline=0, command=self.manager.save_game)
        game.add_command(label="Save game As ...", underline=10, command=self.manager.save_game_as)
        game.add_separator()
        game.add_command(label="Set up Board ...", underline=7, command=self.show_setup_board_dialog)
        game.add_command(label="Flip board", underline=0, command=self.flip_board)
        game.add_separator()
    ...
    def create_options_menu(self):
        ...
        think.add_radiobutton(
            label="1 second", underline=None, command=self.set_think_time, variable=self.thinkTime, value=1
        )
        think.add_radiobutton(
            label="2 seconds", underline=None, command=self.set_think_time, variable=self.thinkTime, value=2
        )
        think.add_radiobutton(
            label="5 seconds", underline=None, command=self.set_think_time, variable=self.thinkTime, value=5
        )
        think.add_radiobutton(
            label="10 seconds", underline=None, command=self.set_think_time, variable=self.thinkTime, value=10
        )
        think.add_radiobutton(
            label="30 seconds", underline=None, command=self.set_think_time, variable=self.thinkTime, value=30
        )
        think.add_radiobutton(
            label="1 minute", underline=None, command=self.set_think_time, variable=self.thinkTime, value=60
        )
        options.add_cascade(label="CPU think time", underline=0, menu=think)
        options.add_separator()
        options.add_command(label="Preferences ...", underline=0, command=self.show_preferences_dialog)
        self.menu_bar.add_cascade(label="Options", menu=options)

    def create_help_menu(self):
        help_menu = Menu(self.menu_bar, tearoff=0)
        help_menu.add_command(label="About Raven ...", underline=0, command=self.show_about_box)
